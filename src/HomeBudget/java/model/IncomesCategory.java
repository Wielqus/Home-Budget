 ...
 */
@Entity
@Table(name = "IncomesCategory")
public class IncomesCategory implements Serializable {
     ...
     * @return List
     */
    public static synchronized List getIncomesCategory() {
        final org.hibernate.Session session = HibernateUtil.getSessionFactory().openSession();
        List results = null;
        try {
            session.beginTransaction();
            results = session.createCriteria(IncomesCategory.class).list();
