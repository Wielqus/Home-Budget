 ...
 * @author Wielq
 */
public class AddincomesController extends BaseController implements Initializable {
    
    public static final String URL_FXML = "/HomeBudget/resources/view/addincomes.fxml";
    ...
    public void initialize(URL url, ResourceBundle rb) {
        ...
        if (list != null) {
            while (list.hasNext()) {
                IncomesCategory incomesCategory = (IncomesCategory) list.next();
                ChoiceBoxItem item = new ChoiceBoxItem(incomesCategory.getId(), incomesCategory.getName());
    ...
    private void saveIncome(ActionEvent event) {
        try {
            ...
            if (id == 0) {
                Incomes.insertIncome(Integer.parseInt(Session.getCurrentSession().get("user")), name, price, category, date);
            }else{
                Incomes.editIncome(id,Integer.parseInt(Session.getCurrentSession().get("user")), name, price, category, date);
            }
     ...
     *
     * @param id-Income id
     * @param name-Income name
     * @param price-Income price
